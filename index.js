  ...
  async handleRequest(request, env, ctx) {
    ...
    <body>
      <div class="container">
          ...
          <!-- [优化] 添加区域HTML结构，并新增排序输入框，Logo 添加魔术棒 -->
<div class="add-new">
  <input type="text" id="addName" placeholder="Name" required style="flex: 2 1 180px; min-width: 150px;">
  <input type="text" id="addUrl" placeholder="URL" required style="flex: 2 1 180px; min-width: 150px;">
  <div class="add-logo-group" style="flex: 2 1 180px; min-width: 150px; display:flex; gap:5px;">
    <input type="text" id="addLogo" placeholder="Logo(可选)" style="flex:1;">
    <button type="button" class="magic-btn" data-url-input="addUrl" data-logo-input="addLogo" style="padding:5px 10px; background:#6c63ff; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:16px;">🪄</button>
  </div>
  <input type="text" id="addDesc" placeholder="描述 (可选)" style="flex: 1 1 80px; min-width: 100px;">
  <input type="text" id="addCatelog" placeholder="分类" required style="flex: 1 1 110px; min-width: 120px;">
  <input type="number" id="addSortOrder" placeholder="排序 (数字小靠前)" style="flex: 0.5 1 80px; min-width: 70px;">
  <button id="addBtn" style="flex: 0 1 auto;">添加</button>
</div>
...
.container {
 ...
 /* [优化] 添加区域适配移动端 */
    ...
    .add-new .add-logo-group {
        flex: 1 1 150px;
        min-width: 150px;
        display: flex;
        gap: 5px;
    }
    .add-new .add-logo-group input {
        flex: 1;
        margin-bottom: 0;
    }
    input[type="text"] {
            ...
            <div class="modal-content">
              ...
              <form id="editForm">
                ...
                <div style="display:flex; gap:5px;">
                  <input type="text" id="editLogo" style="flex:1;">
                  <button type="button" class="magic-btn" data-url-input="editUrl" data-logo-input="editLogo" style="padding:5px 10px; background:#6c63ff; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:16px;">🪄</button>
                </div><br>
        ...
        addSortOrder.value = ''; // [新增]				 
          ...
          document.querySelectorAll('.magic-btn').forEach(btn => {
            btn.addEventListener('click', function(e) {
              ...
              if (urlInputId && logoInputId) {
                magicFillFavicon(urlInputId, logoInputId);
              }
            });
          });
...
<head>
  ...
  <title>洛宸导航</title>
  <link rel="icon" href="https://img.lcit.cc.cd/file/1784204385655_主页.png" type="image/png" />
    ...
    tailwind.config = {
      theme: {
        extend: {
          colors: {
            primary: {
              50:'#f3f5f9',100:'#e1e7f1',200:'#c3d0e3',300:'#9cb3d1',400:'#6c8fba',
              500:'#416d9d',600:'#305580',700:'#254267',800:'#1d3552',900:'#192e45',950:'#101e2d'
            },
            secondary: {
              50:'#fdf8f3',100:'#f6ede1',200:'#ead6ba',300:'#dfc19a',400:'#d2aa79',
              500:'#b88d58',600:'#a17546',700:'#835b36',800:'#6b492c',900:'#5a3e26',950:'#2f1f13'
            },
            accent: {
              50:'#f2faf6',100:'#d9f0e5',200:'#b4dfcb',300:'#89caa9',400:'#61b48a',
              500:'#4f46e5',600:'#4338ca',700:'#3730a3',800:'#312e81',900:'#1e1b4b',950:'#0e103b'
            }
          },
  ...
  <style>
    ...
    ::-webkit-scrollbar-track { background: rgba(237,241,247,0.4); border-radius: 10px; }
    ::-webkit-scrollbar-thumb { background: rgba(195,208,227,0.7); border-radius: 10px; }
    ::-webkit-scrollbar-thumb:hover { background: #416d9d; }

    /* 头部完全清除阴影遮罩 */
    header .absolute { display: none !important; background: transparent !important; }
    ...
    body::after {
      ...
      height: 45vh;
      pointer-events: none;
      background: linear-gradient(to top, rgba(244,248,251,0.75), transparent);
      z-index: -1;
    }
...
<body class="font-sans text-gray-800 bg-fixed bg-cover bg-center min-h-screen" style="background-image: url('https://bing.lcit.ccwu.cc/');">
  <!-- 删除全局灰色遮罩，原图直接显示 -->
  ...
  <!-- 右上角两行时间 -->
  <div class="fixed top-4 right-4 z-50 text-white text-lg text-shadow hidden md:block">
    <div id="dateText" class="block font-medium tracking-wide"></div>
    <div id="timeText" class="block text-3xl md:text-4xl font-bold mt-1" style="font-variant-numeric: tabular-nums;"></div>
  </div>
  ...
  <aside id="sidebar" class="fixed left-0 top-0 h-full w-64 glass-white overflow-y-auto">
    <div class="p-6">
      <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-bold text-primary-600 tracking-tight">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;洛宸导航</h2>
        <button id="closeSidebar" class="p-1 rounded-full hover:bg-gray-200 cursor-pointer z-[60] relative">
      ...
      <div class="mb-6">
        <div class="relative">
          <input id="searchInput" type="text" placeholder="搜索书签..." class="w-full pl-10 pr-4 py-2 border border-primary-100 rounded-lg glass-white focus:outline-none focus:ring-2 transition">
      ...
      <div class="mb-6">
        <h3 class="text-sm font-medium text-gray-500 mb-3">分类快速跳转</h3>
        <div class="space-y-2" id="sidebarCategoryList">
          ${catalogs.map(cat => {
            const safeCat = escapeHTML(cat);
            return `<a href="#cat-${safeCat}" class="block px-3 py-2 rounded glass-white hover:text-primary-600 text-sm">${safeCat}</a>`;
          }).join('')}
        </div>
      </div>
      ...
      <div class="mt-8 pt-6 border-t border-gray-200">
        ${submissionEnabled ? `
        <button id="addSiteBtnSidebar" class="w-full flex items-center justify-center px-4 py-2 bg-accent-500 text-white rounded-lg hover:bg-accent-600 transition shadow-sm">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
          </svg>
          添加新书签
        </button>` : `
        <div class="w-full px-4 py-3 text-xs text-primary-600 glass border border-secondary-100 rounded-lg">访客书签提交功能已关闭</div>`}

        <a href="https://lcbg.cc.cd/" target="_blank" class="mt-4 flex items-center px-4 py-2 text-gray-600 hover:text-primary-500 transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <a href="/admin" target="_blank" class="mt-4 flex items-center px-4 py-2 text-gray-600 hover:text-primary-500 transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
          </svg>
          后台管理
  ...
  <main id="main-content" class="main-content min-h-screen transition-all duration-300">
    <header class="relative overflow-hidden">
      <div class="relative max-w-3xl mx-auto text-center px-4">
        <h1 class="text-4xl md:text-5xl font-bold text-white text-shadow mb-3">上网，从这里开始！</h1>
        <div class="text-white/85 text-sm mb-8 text-shadow">
          <a href="https://hitokoto.cn/" target="_blank" id="hitokoto_text">加载每日一言中...</a>
        </div>
        <div class="glass-white rounded-2xl p-4 border border-white/20">
          <div class="flex items-center gap-2">
            <input id="homeSearchInput" type="text" placeholder="搜索内容..." class="search-input flex-1 px-4 py-3 rounded-xl border border-primary-200 bg-white/90 text-gray-800 text-sm">
            <button id="baiduSearchBtn" class="px-4 py-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 text-sm">百度一下</button>
            <button id="googleSearchBtn" class="px-4 py-3 bg-red-500 text-white rounded-xl hover:bg-red-600 text-sm">Google</button>
          </div>
        </div>
      </div>
    </header>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ${catalogs.map(cat => {
        ...
        return `
          <div id="cat-${safeCat}" class="category-block mb-8">
            <h2 class="text-lg text-white text-shadow mb-3 font-semibold">${safeCat}</h2>
            <div class="rounded-2xl glass-white p-3 sm:p-4">
              <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2 sm:gap-3">
                ${filterSites.map((site) => {
                  ...
                  return `
                    <div class="site-card">
                      <a href="${hrefValue}" ${hasValidUrl ? 'target="_blank" rel="noopener noreferrer"' : ''}>
                        <div class="flex justify-center mb-1">
                          ${logoUrl ? `<img src="${escapeHTML(logoUrl)}" alt="${safeName}" class="w-8 h-8 rounded-lg object-cover">` : `<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-accent-500 to-primary-600 flex items-center justify-center text-white text-xs">${cardInitial}</div>`}
                        </div>
                        <h3 class="text-xs font-medium text-gray-900 truncate">${safeName}</h3>
                      </a>
                    </div>
                  `;
                }).join('')}
    ...
    <footer class="glass-white py-8 px-6 mt-12 border-t border-white/30">
      <div class="max-w-5xl mx-auto text-center">
        <p class="text-gray-600">© ${new Date().getFullYear()}洛宸导航</p>
        <div class="mt-4 flex justify-center space-x-6">
          <a href="" target="_blank" class="text-gray-400 hover:text-primary-500">
  ...
  <!-- 返回顶部 -->
  <button id="backToTop" class="fixed bottom-8 right-8 p-3 rounded-full bg-accent-500 text-white shadow-sm opacity-0 invisible transition-all hover:bg-accent-600">
    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-width="2" d="M5 11l7-7 7 7M5 19l7-7 7 7" />
    </svg>
  </button>

  <!-- 添加站点弹窗 -->
  ${submissionEnabled ? `
  <div id="addSiteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 opacity-0 invisible transition-all">
    <div class="glass-white rounded-xl shadow-lg w-full max-w-md mx-4 transform translate-y-8 transition-all">
      <div class="p-6">
        ...
        <form id="addSiteForm" class="space-y-4">
          <div>
            ...
            <input type="text" id="addSiteName" required class="mt-1 w-full px-3 py-2 border rounded-md glass-white">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700">网址</label>
            <input type="text" id="addSiteUrl" required class="mt-1 w-full px-3 py-2 border rounded-md glass-white">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700">Logo (可选)</label>
            <div class="flex items-center gap-2 mt-1">
              <input type="text" id="addSiteLogo" class="flex-1 px-3 py-2 border rounded-md glass-white">
              <button type="button" class="magic-btn p-2 bg-accent-500 text-white rounded-md hover:bg-accent-600 transition" data-url-input="addSiteUrl" data-logo-input="addSiteLogo" title="自动获取图标">🪄</button>
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700">描述 (可选)</label>
            <textarea id="addSiteDesc" rows="2" class="mt-1 w-full px-3 py-2 border rounded-md glass-white"></textarea>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700">分类</label>
            <input type="text" id="addSiteCatelog" required list="catalogList" class="mt-1 w-full px-3 py-2 border rounded-md glass-white">
            <datalist id="catalogList">${datalistOptions}</datalist>
          </div>
          <div class="flex justify-end pt-4">
            <button type="button" id="cancelAddSite" class="glass-white py-2 px-4 border mr-3">取消</button>
            <button type="submit" class="bg-accent-500 text-white py-2 rounded-md">提交</button>
          </div>
        </form>
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.body.addEventListener('click', function(e) {
    if (sidebar.classList.contains('open') && !sidebar.contains(e.target) && !sidebarOpenBtn.contains(e.target)) {
      closeSidebarMenu();
    }
  });
  ...
  function updateDateTime() {
    ...
    dateEl.textContent = y + "年" + m + "月" + day + " " + week;
  ...
  .then(function(data) {
    const hit = document.getElementById('hitokoto_text');
    hit.href = "https://hitokoto.cn/?uuid=" + data.uuid;
    hit.innerText = data.hitokoto;
  })
  .catch(function() {
    const hit = document.getElementById('hitokoto_text');
    hit.innerText = "每日一言加载失败";
  });

  // 顶部搜索
  const homeSearch = document.getElementById('homeSearchInput');
  const baiduBtn = document.getElementById('baiduSearchBtn');
  const googleBtn = document.getElementById('googleBtn');
  function getKey() {
    return homeSearch ? homeSearch.value.trim() : '';
  }
  if (baiduBtn) {
  ...
  function openModal() {
    ...
    modal.querySelector('.max-w-md').classList.remove('translate-y-8');
    document.body.style.overflow = 'hidden';
  }
  function closeModal() {
    modal.classList.add('opacity-0','invisible');
    modal.querySelector('.max-w-md').classList.add('translate-y-8');
    document.body.style.overflow = '';
  }
  if (addBtn) {
  ...
  if (form) {
    form.addEventListener('submit', async function(e) {
      ...
      const payload = {
        ...
        logo: document.getElementById('addSiteLogo').value.trim(),
        desc: document.getElementById('addSiteDesc').value.trim(),
        catelog: document.getElementById('addSiteCatelog').value.trim()
      };
      ...
      if (data.code === 201) {
        ...
        tip.className = 'fixed top-4 right-4 z-50 bg-accent-500 text-white p-3 rounded';
        tip.innerText = "提交成功，等待管理员审核";
  ...
  if (sideSearch) {
    sideSearch.addEventListener('input', function() {
      const kw = sideSearch.value.toLowerCase().trim();
      cards.forEach(function(card) {
        const n = card.querySelector('h3').innerText.toLowerCase();
        const c = card.closest('.category-block').querySelector('h2').innerText.toLowerCase();
        if (n.indexOf(kw) > -1 || c.indexOf(kw) > -1) {
